---
title: "Exploratory data analysis (II): Exploring Numerical Data"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
- The car names are character strings, which are like factors, except its common for every case to take a unique value. 
- **`lgl`**, stands for logical variables, another simple case of a categorical variable where there are only two levels. For example, each car will take either TRUE or FALSE depending on if it is a sports car or not. 
...
```{r hist-try, exercise=TRUE, warning=TRUE, echo = TRUE}
...
You may select the rows with missing values.

```{r, na-try, exercise=TRUE}
cars[is.na(cars$weight),]
```


### Histogram bins 

You can play around with the `bins` or `binwidth` arguments to find the number of bins to make the histogram as continuous as possible. 
...
The `bins` and `binwidth` arguments to `geom_histogram()` is similar to the `dotsize` argument to `geom_dotplot()`, as it is optional and controls one aspect of the histogram. W may choose to specify the number of bins we would like, instead of the 30 bins `geom_histogram()` defaults to. 

Create histograms of car width with `bindwidth` 15 and 50

```{r wbw-ex, exercise=TRUE, echo = TRUE}
ggplot(data = ____ , aes(x = ______)) +
  geom_histogram(bins = ______)+
  ggtitle("Histogram of car width with bindwidth of 15") +

ggplot(data = ______ , aes(x = ______)) +
  geom_histogram(bins = ______)+
  ggtitle("Histogram of car width with bindwidth of 50") +
```

```{r wbw-ex-solution}
ggplot(data = cars, aes(x = weight)) +
  geom_histogram(bins = 10) +
  ggtitle("Histogram of car with 10 bins") 

ggplot(data = cars, aes(x = weight)) +
  geom_histogram(bins = 40) +
  ggtitle("Histogram of car with 40 bins") 
```
...
```{r dotplot-cars-try, exercise=TRUE,  echo = TRUE}
ggplot(data = cars, aes(x = weight)) +
...
Add appropriate titles to the plots
...
```{r cars-density-plot-try, exercise=TRUE, exercise.eval=TRUE, echo=TRUE}
ggplot(data = cars, aes(x = weight)) +
  geom_density()
```
...
You can think of this as a "filling" the density plots with different colors for sports car and other types of cars. 
...
```{r cars-colored-try, exercise = TRUE}
ggplot(data = cars, aes(x = weight, fill = sports_car)) +
  geom_density(alpha = 0.5)
```

### Try it  yourself! 

MPG is an abbreviation for 'miles-per-gallon' and it is used to give you an indication of the fuel economy of a car or commercial vehicle. The miles per gallon of a car tends to be associated with the size of its engine (as measured by the number of cylinders). 
...
Keep in mind that `ncyl` is a numerical variable, so you will need to convert it to a categorical variable to use it as the `fill` using the `as.factor()` function. 

```{r ex2, exercise = TRUE, fig.height=4, fig.width=6}
...
```{r cars-boxplot-try, exercise=TRUE, echo=TRUE}
ggplot(data = cars, aes(x = weight)) +
  geom_boxplot()
```
...
```{r dp2, exercise=TRUE, echo=TRUE, eval = FALSE}
cars %>%
  filter(eng_size < 2.0) %>%
...
Consider two columns in the `cars` dataset: `city_mpg` and `width`. Which is the most appropriate plot for displaying the important features of their distributions?
...
```{r hist-hwy-mpg-pickup, echo=TRUE}
ggplot(data = cars, aes(x = hwy_mpg)) +
  geom_histogram() +
...
Let's investigate the distribution of the fuel efficiency (`city_mpg`) faceted by whether the vehicle is classified as an `suv` (a logical variable indicating whether the car is a sport utility vehicle (SUV) or not).
...
```{r ex7-setup-text, echo=TRUE}
common_cyl <- filter(cars, ncyl %in% c(4, 6, 8))
```


```{r ex7-setup, echo=TRUE}
common_cyl <- filter(cars, ncyl %in% c(4, 6, 8))
```
