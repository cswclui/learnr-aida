...
The first variable is id, which is an identifier variable for the student. Strictly speaking, this is a categorical variable, though the labeling of this variable is likely not that useful since we would not use this variable in an analysis of relationships between the variables in the dataset. 
...
`"female"`. There is no inherent ordering to the levels of this variable, no matter what
anyone tells you! So, this is just a categorical variable.
...
```{r hsb2-science-math-lm, exercise=TRUE}
ggplot(data = hsb2, aes(x = science, y = math)) +
  geom_point() +
...
```{r hsb2-science-mat-prog-lm, exercise=TRUE}
ggplot(data = hsb2, aes(x = science, y = math, color = prog)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
```
...
```{r, echo=TRUE}
ggplot(email50, aes(x =  num_char, fill = factor(spam))) +
  geom_density(alpha=0.3)
```

```{r, echo=TRUE}
ggplot(email50, aes(x =  dollar, fill = factor(spam))) +
  geom_density(alpha=0.3)
```


```{r, echo=TRUE}
ggplot(email, aes(x =  num_char, y=spam, color = factor(spam))) +
  geom_point()
```

```{r, echo=TRUE}
ggplot(email, aes(x =  dollar, y=spam, color = factor(spam))) +
  geom_point()
```

```{r, echo=TRUE}
ggplot(email, aes(x =  exclaim_mess, y=spam, color = factor(spam))) +
  geom_point()
```
