...
You can learn more about  `ggplot` function by bringing up the help file using the command `?ggplot`.
...
To compute the total number of baptisms in each year, we can add the vector for baptisms for boys to that of girls

```{r calc-total-bapt-vars, exercise = T}
arbuthnot$boys + arbuthnot$girls
```

You will see are 82 numbers (in that packed display, because we aren’t looking at a data frame here), each one representing the sum we’re after. 


To compute  the total number of baptism in 1629, execute

```{r baptism-ex1, exercise = T}
arbuthnot$boys[1] + arbuthnot$girls[1]
```


**Exercise:** What is the total number of baptism in 1640?

```{r baptism-ex2, exercise = TRUE}


```


```{r baptism-ex2-hint, eval=FALSE}
# The function which(arbuthnot$year == 1640) will return the row index for the year 1640.

```

```{r baptism-ex2-solution}
i <- which(arbuthnot$year == 1640)
arbuthnot$boys[i] + arbuthnot$girls[i]
```
...
- All other columns in the `arbuthnot` data frame have values that are numerical (the year, the number of boys and girls).  Here, we've asked R to create *logical* data, data where the values are either `TRUE` or `FALSE`. 
...
To find the minimum and maximum values of columns, you can use the functions `min` and `max` within a `summarise()` call, which you will learn more a   bout in the following lab. 
...
Consider the `present` dataset which stores the numbers of boys and girls born in the United States between 1940 and 2002. 
```{r present-data, exercise =T}
present
```

You can learn more about them by bringing up the help file using the command `?present`.
```{r present-data2, exercise =T}
?present
```
...
What are names of the column in the `present` dataset?
...
In what year did we see the highest total number of births in the US.? 
*Hint:* First calculate the totals and save it as a new variable. Then, sort your
...
What is the year with highest total number of birth in US?

```{r highestbirth, exercise = TRUE}

```


```{r highestbirth-check}
grade_this({
  if (.result == 1961) {
    pass("Correct!")
  } 
